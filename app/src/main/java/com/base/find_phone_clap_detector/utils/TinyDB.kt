...
class TinyDB(appContext: Context?) {
    ...
    fun getListObject(key: String?, mClass: Class<*>?): ArrayList<SoundsDataClass> {
        val gson = Gson()
        val objStrings = getListString(key)
        val objects = ArrayList<SoundsDataClass>()
        for (jObjString in objStrings) {
            val value = gson.fromJson(jObjString, mClass)
            objects.add(value as SoundsDataClass)
        }
        return objects
    }
...
//            throw new NullPointerException();
    ...
    fun putListObject(key: String?, objArray: ArrayList<SoundsDataClass>) {
        checkForNullKey(key)
        val gson = Gson()
